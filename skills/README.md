...
│       ├── setup.md                # deploy / undeploy / version matching
...
| Framework | Location |
|---|---|
| Claude / Claude Code | your project or user skills directory (each skill = a folder with `SKILL.md`) |
...
- **Kubernetes skill:** the `kubectl gadget` plugin locally + the IG DaemonSet
  ...
  `kubernetes-troubleshooting/references/setup.md`.
- **Linux skill:** the `ig` binary + root (CAP_BPF/CAP_SYS_ADMIN) + a kernel with
  BTF (`/sys/kernel/btf/vmlinux`). See `linux-troubleshooting/references/setup.md`.
