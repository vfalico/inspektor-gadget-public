...
Upstream auto-generates its gadget doc flag tables from gadget metadata — the
project treats the gadget as the single source of truth for its own interface.
This skill follows the same principle: **teach the agent to read the gadget, not
to memorize it.** That is why this skill deliberately does not embed exhaustive
per-gadget field lists — they would drift, and the gadget answers authoritatively
in one command.
