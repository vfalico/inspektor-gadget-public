...
sequenceDiagram
    ...
    participant Kernel as System Calls
    participant eBPF as eBPF Probes
    participant Traceloop as Traceloop Recorder
    participant Output as Output Stream
    participant User as Traceloop User
    App ->> Kernel: Generate syscalls
    Kernel ->> eBPF: Kernel level interception
    eBPF ->> Traceloop: Capture events
    Traceloop ->> Output: Buffer & format data
