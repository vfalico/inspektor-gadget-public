...
`snapshot_file` walks open files with a BPF task/file iterator (`SEC("iter/task_file")`), which requires Linux 5.8 or later — the same baseline as the sibling snapshot gadgets `snapshot_process` and `snapshot_socket`.
