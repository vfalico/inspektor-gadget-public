...
`bpfstats` reports the CPU time and memory used by each eBPF program. This relies on BPF runtime statistics, enabled through `BPF_ENABLE_STATS`, which was introduced in Linux 5.8.
