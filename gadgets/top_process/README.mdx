...
`top_process` gathers process statistics from procfs via the userspace `process` operator and does not attach any eBPF program, so it has no special kernel-feature requirement beyond the Inspektor Gadget baseline.
...
interval to re-read statistics
