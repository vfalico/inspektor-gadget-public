# Driving MEP over MCP (agent / tool-call interface)

MEP can be driven two ways: the `ig` CLI (everything else in this skill), or as an
...
- **CLI** — you're at a shell, doing interactive RCA, or scripting `mep-run.sh`.
- **MCP** — an MCP-capable assistant is doing the RCA and should pick the capability,
  ...
  anything; MEP stays a single read-only gadget.
...
2. **`mep_coverage` first.** Every call returns exactly one coverage record
...
3. Inspect `mep_coverage`: attached but idle → re-call with a wider `timeout`.
...
- Errors surface in `mep_coverage.note` / a non-zero attach — surface them to the
